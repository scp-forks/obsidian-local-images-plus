...
export default class LocalImagesPlugin extends Plugin {
  ...
  async onload() {
    ...
    this.app.vault.on("delete", async (file: TFile) => {
      ...
      if (
        this.settings.saveAttE !== "obsFolder" &&
        ...
        try {
          if (
            this.app.vault.getAbstractFileByPath(rootdir) instanceof TFolder
          ) {
    ...
    this.app.vault.on("rename", async (file: TFile, oldPath: string) => {
      ...
      if (
        ...
        oldRootdir = oldRootdir.replace(
          "${notename}",
          path.parse(oldPath)?.name
        );
        let newRootDir = oldRootdir.replace(
          path.parse(oldPath)?.name,
        ...
        try {
          if (
            this.app.vault.getAbstractFileByPath(oldRootdir_) instanceof TFolder
          ) {
        ...
        content = content
          .replaceAll(
            "](" + encodeURI(oldRootdir),
            "](" + encodeURI(newRootDir)
          )
    ...
    this.app.vault.on("modify", async (file: TFile) => {
      ...
      if (!file || !(file instanceof TFile) || !this.ExemplaryOfMD(file.path)) {
        return;
      } else {
  ...
  private async processPage(
    ...
    logError(`Preparing to process ${file.path}`, false);
    ...
    async () => {
      ...
      if (type == "plugin") {
        ...
        if (this.ExemplaryOfMD(noteFile.path)) {
          oldRootdir = oldRootdir.replace(
            "${notename}",
            path.parse(noteFile.path)?.name
          );
          oldRootdir = trimAny(
            pathJoin([path.parse(noteFile.path)?.dir, oldRootdir]),
            ["/"]
          );
          ...
          const allAttachments = await this.app.vault.getAbstractFileByPath(
            oldRootdir
          )?.children;
          ...
          if (allAttachments) {
            for (const attach of allAttachments) {
              if (
                !allAttachmentsLinks.includes(attach.name) &&
                attach.children == undefined
              ) {
          ...
          if (orphanedAttachments.length > 0) {
            ...
            mod.messg =
              ...
              "'\r\n\r\n      ";
            mod.plugin = this;
            mod.callbackFunc = this.removeOrphans(
              "execremove",
              orphanedAttachments
            );
            mod.open();
      ...
      if (type == "obsidian") {
        ...
        const allAttachments =
          this.app.vault.getAbstractFileByPath(obsmediadir)?.children;
        let orphanedAttachments = [];
        let allAttachmentsLinks = [];
              ...
              if (canvasData.nodes && canvasData.nodes.length > 0) {
                for (const node of canvasData.nodes) {
                  ...
                  if (node.type === "file") {
                    ...
                    const AllNodeLinks = (
                      await this.app.internalPlugins.plugins.canvas.instance.index.parseText(
                        node.text
                      )
                    )?.links;
      ...
      if (type == "execremove") {
        const useSysTrash =
          this.app.vault.getConfig("trashOption") === "system";
        ...
        showBalloon(
          filesToRemove.length + " file(s) " + msg,
          this.settings.showNotifications
        );
      }
    };
        ...
        if (embeds || pr) {
          ...
          for (let el of embeds) {
            ...
            if (
              (this.newfCreated.indexOf(el.link) != -1 ||
                (obsmdir != "" &&
                  (this.newfCreated.includes(oldpath) ||
                    this.newfCreated.includes(el.link)))) &&
              ...
              let newlink: Array<string> = await getRDir(
                note,
                this.settings,
                newpath
              );
              ...
              if (this.settings.PngToJpegLocal && fileExt == "png") {
                ...
                const blob = new Blob([
                  new Uint8Array(
                    await this.app.vault.adapter.readBinary(oldpath)
                  ),
                ]);
                newBinData = await blobToJpegArrayBuffer(
                  blob,
                  this.settings.JpegQuality * 0.01
                );
                ...
                if (newBinData != null) {
                  if (this.settings.useMD5ForNewAtt) {
                    ...
                    newpath = pathJoin([
                      mdir,
                      cFileName(path.parse(el.link)?.name + ".jpeg"),
                    ]);
                  }
              ...
              if (await this.app.vault.adapter.exists(newpath)) {
                ...
                if (newBinData != null) {
                  newFMD5 = md5Sig(
                    await this.app.vault.adapter.readBinary(newpath)
                  );
                } else {
                  newFMD5 = md5Sig(
                    await readFromDiskB(
                      pathJoin([this.app.vault.adapter.basePath, newpath]),
                      5000
                    )
                  );
                }
                ...
                if (
                  newMD5 === newFMD5 ||
                  (oldMD5 === newFMD5 && oldpath != newpath)
                ) {
                  ...
                  while (await this.app.vault.adapter.exists(newpath)) {
                    newpath = pathJoin([
                      mdir,
                      `(${inc}) ` + cFileName(path.basename(el.link)),
                    ]);
                    inc++;
                  }
                ...
                try {
                  if (newBinData != null) {
                    await this.app.vault.adapter
                      .writeBinary(newpath, newBinData)
                      .then();
                    {
                      await this.app.vault.adapter.remove(oldpath);
                    }
              ...
              if (this.settings.addNameOfFile) {
                if (useMdLinks) {
                  addName = `[Open: ${path.basename(el.link)}](${
                    newlink[1]
                  })\r\n`;
                } else {
                  addName = `[[${newlink[0]}|Open: ${path.basename(
                    el.link
                  )}]]\r\n`;
                }
              }
              ...
              if (useMdLinks) {
                newtag =
                  addName + oldtag.replace(encObsURI(el.link), newlink[1]);
              }
  ...
  private setTitleAsName = async () => {
    try {
      ...
      var ind = 0;
      if (title !== null) {
        const newName = cFileName(
          trimAny(title[0].toString(), ["#", " "])
        ).slice(0, 200);
        var fullPath = pathJoin([noteFile.parent.path, newName + ".md"]);
        var fExist = await this.app.vault.exists(fullPath);
        if (
          trimAny(noteFile.path, ["\\", "/"]) != trimAny(fullPath, ["\\", "/"])
        ) {
          while (fExist) {
            ind++;
            var fullPath = pathJoin([
  ...
  private convertSelToURI = async () => {
    this.app.workspace.activeEditor.editor.replaceSelection(
      encObsURI(await this.app.workspace.activeEditor.getSelection())
    );
  };

  private convertSelToMD = async () => {
    this.app.workspace.activeEditor.editor.replaceSelection(
      htmlToMarkdown(await this.app.workspace.activeEditor.getSelection())
    );
  };
