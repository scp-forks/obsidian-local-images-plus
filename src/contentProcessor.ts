  ...
  async function processImageTag(
    ...
    try {
      ...
      if (protocol == "data:") {
        ...
        // Parse the URL properly to handle all OS formats
        try {
          const fileUrl = new URL(link);
          fpath = decodeURIComponent(fileUrl.pathname);

          // macOS specific handling - pathname starts with '/' which needs to be retained
          if (SUPPORTED_OS.unix.includes(opsys) && fpath.startsWith('/')) {
            // Keep the path as is, it's already correct for Unix-like systems
            logError("macOS/Unix path format detected: " + fpath);
          } else if (SUPPORTED_OS.win.includes(opsys)) {
            // For Windows, remove the leading slash
            fpath = fpath.replace(/^\//, '');
            logError("Windows path format detected: " + fpath);
          }

          logError("Parsed file path: " + fpath);
        } catch (e) {
          // Fallback to old behavior if URL parsing fails
          logError("URL parsing failed, using fallback method");
        ...
        logError("Attempting to read from path: " + fpath);
        fileData = await readFromDisk(fpath);

        if (fileData === null) {
          // Try with decoded URI
          const decodedPath = decodeURI(fpath);
          logError("First attempt failed, trying with decoded path: " + decodedPath);
          fileData = await readFromDisk(decodedPath);

          if (fileData === null && !fpath.startsWith('/')) {
            // Try adding a leading slash for Unix systems
            logError("Second attempt failed, trying with leading slash: /" + fpath);
            fileData = await readFromDisk('/' + fpath);
          }
        }
      } else {
      ...
      try {
        ...
        if (fileName) {
          ...
          if (!app.app.vault.getConfig("useMarkdownLinks")) {
            ...
            !settings.useCaptions || !imgsize.length
              ? (caption = "")
              : (caption = "|" + imgsize);
